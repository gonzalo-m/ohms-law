 ...
 */
public class MyCheckBoxListPreference extends ListPreference {
	...
	public MyCheckBoxListPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public MyCheckBoxListPreference(Context context) {
        this(context, null);
    }
    ...
    protected void onPrepareDialogBuilder(Builder builder) {
        ...
        /* Solves problem with cancel button 
		 * save previous state */
        savePrevEntryValues();
    ...
    private void restoreCheckedEntries() {
    	...
    	String[] vals = null;
    	if ( getValue() != null ) 
    	vals = parseStoredValue(getValue());
...
//        super.onDialogClosed(positiveResult);
        ...
        /* When dialog is canceled recover entries to previous state */    
        } else {
        	for (int i = 0; i < mClickedDialogEntryIndices.length; i++) {
        		mClickedDialogEntryIndices[i] = previousEntryValues[i];
        	}
        }
    }
	...
	// TODO: Would like to keep this static but separator then needs to be 
	// put in by hand or use default separator "OV=I=XseparatorX=I=VO"...
	...
	private boolean[] previousEntryValues;
	private void savePrevEntryValues() {
		previousEntryValues = new boolean[mClickedDialogEntryIndices.length];
		for (int i = 0; i < mClickedDialogEntryIndices.length; i++) {
			previousEntryValues[i] = mClickedDialogEntryIndices[i];
		}
	}
}
